...
async function main() {
  const srcPath = path.join(__dirname, "fare-route-restrictions.json");
  const outDir = path.join(__dirname, "fare-route-restrictions-batches");
  const batchSize = 50;
  ...
  for (let i = 0; i < codes.length; i += batchSize) {
    ...
    const fileName = `fare-route-restrictions-batch-${batchIndex + 1}.json`;
